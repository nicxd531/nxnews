                    ...
                    <Box key={index} sx={{ height: "16%", width: "100%", display: "flex" }}>
                        <Link href={`/post/${data?._id}/${data?.slug}`}>
                            <IconButton sx={{ textAlign: "start", color: "inherit", p: 0, borderRadius: 0, height: "100%", width: "100%" }}>
                                ...
                                <Box sx={{ ml: 2 }}>
                                    <Typography sx={{ mb: { xs: 0 } }}>{data.mainHeading}</Typography>
                                    <Typography sx={{ mt: 3, fontSize: { xs: "0.7rem", lg: "1rem" } }}>{data.createdAt} <span style={{ color: "grey" }}>-15 minutes ago</span></Typography>
                                </Box>
                            </IconButton>
                        </Link>
                    </Box>
