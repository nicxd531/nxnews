...
function HomeSection11({ post, error }) {
    // home section 11 and const for holding filtered data , sliced data and map data
    const filteredData = post?.filter((data) => data.categories == "Business")
            ...
            {(Array.isArray(filteredData) && filteredData?.length > 0) ? (
                ...
                <>
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 4 }}>
                        <Typography variant='h4' sx={{ fontWeight: "bold", fontSize: { xs: "1.3rem", lg: "2.5rem" } }}> Business</Typography>
                        <Button variant='contained' sx={{ textTransform: "capitalize", fontSize: { xs: "0.7rem", lg: "1rem" } }}><Link href={"/business"}>View All</Link> </Button>
                    </Box>
                    <hr />
